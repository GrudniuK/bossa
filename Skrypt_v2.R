...
for(i in 1:nrow(baza)) {
  ...
  #url_source <- paste0(path,'/_CSV/',gsub('\\.','_',tolower(as.character(baza$ticker[i]))),'_d.csv')
...
for(i in 1:nrow(baza)) {
  ...
  if(!is.na(baza$porownanie[i])) {
    ...
    url_source <- paste0(path,'/_CSV/',gsub('\\.','_',tolower(as.character(baza$porownanie[i]))),'_d.csv')
    dane <- read.table(file = url_source, header = TRUE, sep = ",", na.strings = "NA", fill = TRUE, stringsAsFactors = F)
    dane$Date <- as.Date(dane$Date , "%Y-%m-%d")
    if(ncol(dane) < 6) {dane$Volume = 0}
    stooq_por[[i]] <- dane
...
baza$close_obecny <- obecny_stan$Close
...
for(i in 1:nrow(baza)) {
  #i <- 1
  walor <- stooq[[i]]
  atr_5 <- as.data.frame(ATR(walor[,c("High","Low","Close")], n=5))
  atr_14 <- as.data.frame(ATR(walor[,c("High","Low","Close")], n=14))
  atr_21 <- as.data.frame(ATR(walor[,c("High","Low","Close")], n=21))
  
  atr_add = data.frame(
    atr_5 = atr_5[nrow(atr_5), 'atr'],
    atr_14 = atr_14[nrow(atr_14), 'atr'],
    atr_21 = atr_21[nrow(atr_21), 'atr']
  )
  
  if(exists("atr")) {atr <- rbind(atr, atr_add)}
  if(!exists("atr")) {atr <- atr_add}
  rm(walor, atr_5, atr_14, atr_21, atr_add); gc()
}
baza <- cbind(baza, atr)
